...
function initialize(passport) {
    ...
    return function* passportInitialize(next) {
        ...
        ctx.logout = ctx.logOut = ctx.req.logout = ctx.req.logOut = logout.bind(
            req
        );
        ctx.isAuthenticated = ctx.req.isAuthenticated = ctx.req.isAuthenticated.bind(
            req
        );
        ctx.isUnauthenticated = ctx.req.isUnauthenticated = ctx.req.isUnauthenticated.bind(
            req
        );
...
function authenticate(passport, name, options, callback) {
    ...
    if (callback) {
        if (callback.constructor.name !== 'GeneratorFunction') {
            throw TypeError(
                'Your custom authentication callback must be a Generator Function'
            );
        }
